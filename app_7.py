                ...
                if missing:
                    ...
                    for idx, row in df_upload.iterrows():
                        ...
                        if norm_u_comp in comp_norm_db:
                            master_biz_num = comp_norm_db[norm_u_comp]["biz_num"]
                            if u_biz_num != master_biz_num:
                                reasons.append(f"사업자번호 불일치 (입력: {u_biz_num} / 등록: {master_biz_num})")
                        else:
                            if u_company:
                                reasons.append(f"미등록 업체 (오타 확인 필요): {u_company}")

                        if not reasons:
                            formatted_row = []
                            for cell in row_data_for_save:
        ...
        if st.button("🔍 결과 산출하기", use_container_width=True):
            try:
                ...
                if not maintenance_df.empty:
                    ...
                    if period_m_df.empty and not maintenance_df.empty:
                        # 유지보수도 파싱 실패 시 보이도록
                        pass
